...
#    print(i[-16:-8])
    if i[-16:-8] == 'Line 50x':
        j = "Data/train_cleaned/"+str(k1)+'.tif'
        if k1 >40 :
            j = "Data/test_cleaned/"+str(k1)+'.tif'
        shutil.copyfile(i,j)
        k1=k1+1      
        print(i)
for i in file_list:
    if i[-16:-8] == 'Line off':
        j = "Data/train/"+str(k2)+'.tif'
        if k2 >40 :
            j = "Data/test/"+str(k2)+'.tif'
        shutil.copyfile(i,j)
        k2=k2+1
        print(i)        
...
X_test  = renormal_load_image_from_dir('Data/test')
X_test  = X_test.reshape(-1,target_height,target_width,1)

X_test_target  = renormal_load_image_from_dir('Data/test_cleaned')
