## For the evaluation of DDIM
## 
## Based on https://keras.io/examples/generative/ddim/ and modified by Seoleun Shin (KRISS) 2023/May/01 ##
##
...
os.environ['XLA_FLAGS'] = '--xla_gpu_cuda_data_dir=/apps/cuda/11.4'
...
def renormal_load_image_from_dir4(img_path):
    ...
    for i, fig in enumerate(file_list):
        ...
        img_array = tf.image.resize(img_array, size=[image_size, image_size], antialias=True)
...
GCS_PATH = 'Data'
#pathfn_photo = ['/scratch/x2521a01/data/time/high/*.tif']
#pathfn_photo1 =['/scratch/x2521a01/data/time/hightest/*.tif']
#pathfn_noise = ['/scratch/x2521a01/data/time/low/*.tif']
...
#fn_photo = pathfn_photo
#fn_photo1 = pathfn_photo1
#fn_noise = pathfn_noise
...
def tiff_image_generator(filename):
    for path in filename:
        ...
        #image = cropImage(image,(0,0),(400,400))
...
#numpy_dataset = np.array(list(test_dataset.as_numpy_iterator()))

# print the shape of the NumPy array
#print(numpy_dataset.shape)


#print("SIZE=", test_tf_dataset.shape)

# define a function to add shape information to each element
testel = test_dataset.take(1)

print(testel)
#for element in testel:
#    print('CHECKCHECK=',tf.shape(element[:2]))
#    partelement = element[:2]
#    #print(tf.shape(partelement))
#    print(partelement[0,:,:,:])
...
class DiffusionModel(keras.Model):
    ...
    def testgenerate(self, batch_images, diffusion_steps):
        file_list = renormal_load_image_from_dir4('Data/Clean/41.tif')
        noisy_images = renormal_load_image_from_dir4('Data/Noisy/41.tif')
        initial_noise = self.normalizer(noisy_images, training=False)
        generated_images = self.reverse_diffusion(initial_noise, diffusion_steps)
        generated_images = self.denormalize(generated_images)
        return generated_images
    ...
    def testplot_images(self, epoch=None, logs=None, num_rows=1, num_cols=3):
        ...
        testelement = testel
        noisy_images = renormal_load_image_from_dir4('Data/Noisy/41.tif')
        clean_images = renormal_load_image_from_dir4('Data/Clean/41.tif')


        targetx = np.squeeze(clean_images,  axis=0)
        testx = np.squeeze(noisy_images,  axis=0)
        generated_images_gray = np.squeeze(generated_images,  axis=0)
        testout = generated_images_gray[:,:,0]
        predout = np.reshape(testout, [image_size, image_size,1])

        psnrsk_noise, psnrsk_denoised = evaluation(targetx, testx, metrics=["psnr"]), evaluation(targetx, predout, metrics=["rmse"])

        ssim_noise, ssim_denoised = compssim(targetx, testx, metrics=["ssim"]), evaluation(targetx, predout, metrics=["ssim"])
        ...
        psnrcheck_noise, psnrcheck_denoised = evaluation(targetx, testx, metrics=["psnr"]), evaluation(targetx, predout, metrics=["psnr"])

        avgpsnr = np.mean(psnrsk_denoised)
        avgpsnrcheck = np.mean(psnrcheck_denoised)
        avgssim = np.mean(ssim_denoised)
        avgfsim = np.mean(fsim_denoised)
        avguiq = np.mean(uiq_denoised)
        avgrmse = np.mean(rmse_denoised)
        print('psnr=', avgpsnr, 'ssim=', avgssim, 'psnrcheck=', avgpsnrcheck, 'fsim=', avgfsim, 'uiq=', avguiq, 'rmse=', avgrmse)
        ...
        fig.tight_layout()
        ax[0].imshow(testx, cmap=plt.cm.gray, aspect='0.78125')
        ax[1].imshow(targetx, cmap=plt.cm.gray, aspect='0.78125')
        ax[2].imshow(predout, cmap=plt.cm.gray, aspect='0.78125')
        ax[0].axis('off')
        ...
        fig.savefig('./revival_diffimp_test_standard.png')
        plt.close()
