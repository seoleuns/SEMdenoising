...
for i in gile_list:
    ...
    if k1 < 70:
        ...
        jj = "./Gen_IMG/CycleGAN/train_cleaned/"+str(k1)+'.tif'
        shutil.copyfile(j,jj)
    else:
        ii = "./Gen_IMG/CycleGAN/test/"+str(k2)+'.tif'
        shutil.copyfile(i,ii)
        jj = "./Gen_IMG/CycleGAN/test_cleaned/"+str(k2)+'.tif'
        shutil.copyfile(j,jj)
        k2=k2+1
    k1=k1+1 
...
for i in file_list:
    ...
    img=cropImage(img0,(0,0),(440,480))
    imageio.imwrite(j,img)
    if True:
        img1=cropImage(img0,(0,0),(440,480))
        j1=i ; k=random.randint(1000,1000000) ; k1=random.randint(1000,1000000)
...
#photo_img=cropImage(photo_img,(0,0),(255,255))
...
def create_gif(num_photo=0, load=False):
    if load == True:
        anim_file = './Gen_IMG/CycleGAN/CycleGAN.gif'
    else:
